...
from utils import RandomVirt

class RandomVirtSearch:
    def __init__(self, model_file:str, fim_file:str, pop_size:int, max_params:int, n_objs:int):
    ...
    def setup(self, cfg):
        ...
        self.problem = RandomVirt(pop_size=self.pop_size, n_var=self.n_var, min_pagesize=10, max_pagesize=10000, 
                                       max_epochs=cfg.epochs, model=cfg.model, fim=self.fim, 
    ...
    def run_experiment(self, cfg):
        ...
        # pickle.dump(res, f)
        #
        res_path = os.path.join(self.out_dir, 'res')
        np.save(res_path, res)
        var_path = os.path.join(self.out_dir, 'var')
        ...
        # torch.save(self.fim, os.path.join(self.out_dir, f'fim.pt'))
        mlflow.log_artifact(res_path+'.npy')
        mlflow.log_artifact(var_path+'.npy')
