...
class VirtBinarySearch:
    ...
    def run_experiment(self, cfg):
        ...
        for i in range(2):
            ...
            while(True):
                ...
                if test_acc >= best_acc:
                    self.min = page_size if i == 0 else self.min
                    self.max = page_size if i == 1 else self.max
                    best_acc, best_page_size = test_acc, page_size
                else:
                    self.min = page_size if i == 1 else self.min
                    self.max = page_size if i == 0 else self.max
                if page_size == self.min or page_size == self.max:
                    break
            best_accs.append(best_acc)
            best_page_sizes.append(best_page_size)
        best_acc = max(best_accs)
        best_page_size = best_page_sizes[best_accs.index(best_acc)]
        ...
        n_virtualpages = int((((1-self.amount)*self.orig_model.fff.w1s.numel() + best_page_size - 1)//best_page_size))
        ...
        logging.info(f"Orig acc: {orig_test_acc}, acc after weight virt.: {best_acc}")
