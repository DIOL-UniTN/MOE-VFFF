...
class IFFF(nn.Module):
    def __init__(self,
            ...
            region_leak: float=0.0, usage_mode: str = 'none',
                 twin_leaf_indices:List[int]): # Twin indices (idx, idx+1)
        # NOTE: Will still opt. pruned nodes. would that create an issue? (I don't think so tho)
        super().__init__()
        ...
        self.twin_leaf_indices = twin_leaf_indices
