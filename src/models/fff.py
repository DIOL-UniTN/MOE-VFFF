...
class BaseFFF(nn.Module):
    def __init__(self, in_channels, image_size, leaf_width, output_width, depth, dropout, region_leak):
        super(BaseFFF, self).__init__()
        input_width = (in_channels * image_size[0] * image_size[1])
        self.fff = FFF(input_width, leaf_width, output_width, depth, nn.ReLU(), dropout, 
                       train_hardened=False, region_leak=region_leak)

    def forward(self, x, leaf_ids:bool=False):
        x = x.view(len(x), -1)
        x = self.fff(x, leaf_ids=leaf_ids)
        return x

    def parameters(self):
        return self.fff.parameters()

class BaseFFFD2(nn.Module):
    def __init__(self, in_channels, image_size, leaf_width, output_width, depth, dropout, region_leak):
        super(BaseFFFD2, self).__init__()
        input_width = (in_channels * image_size[0] * image_size[1])
        self.fff = FFFD2(input_width, leaf_width, output_width, depth, nn.ReLU(), dropout, 
                       train_hardened=False, region_leak=region_leak)
...
class FisherFFF(nn.Module):
    ...
    def eval_forward(self, x: torch.Tensor, leaf_ids:bool=False) -> torch.Tensor:
        ...
        if leaf_ids:
            return new_logits.view(*original_shape[:-1], self.output_width), leaves # (..., self.output_width)
        return new_logits.view(*original_shape[:-1], self.output_width), leaves # (..., self.output_width) # FOR FIM

class FFFD2(nn.Module):
    def __init__(self,
            input_width: int, leaf_width: int, output_width: int, depth: int,
            activation=nn.ReLU(), dropout: float=0.0, train_hardened: bool=False, 
            region_leak: float=0.0, usage_mode: str = 'none'):
        ...
        self.w1s = nn.Parameter(torch.empty((self.n_leaves, input_width, leaf_width), dtype=torch.float).uniform_(-l1_init_factor, +l1_init_factor), requires_grad=True)
        self.b1s = nn.Parameter(torch.empty((self.n_leaves, leaf_width), dtype=torch.float).uniform_(-l1_init_factor, +l1_init_factor), requires_grad=True)
        self.w2s = nn.Parameter(torch.empty((self.n_leaves, leaf_width, leaf_width), dtype=torch.float).uniform_(-l1_init_factor, +l1_init_factor), requires_grad=True)
        self.b2s = nn.Parameter(torch.empty((self.n_leaves, leaf_width), dtype=torch.float).uniform_(-l1_init_factor, +l1_init_factor), requires_grad=True)
        self.w3s = nn.Parameter(torch.empty((self.n_leaves, leaf_width, output_width), dtype=torch.float).uniform_(-l2_init_factor, +l2_init_factor), requires_grad=True)
        self.b3s = nn.Parameter(torch.empty((self.n_leaves, output_width), dtype=torch.float).uniform_(-l2_init_factor, +l2_init_factor), requires_grad=True)
        self.leaf_dropout = nn.Dropout(dropout)
    ...
    def training_forward(self, x: torch.Tensor, return_entropies: bool=False, use_hard_decisions: bool=False):
        ...
        element_logits = torch.matmul(x, self.w1s.transpose(0, 1).flatten(1, 2))            # (batch_size, self.n_leaves * self.leaf_width)
        element_logits = element_logits.view(batch_size, self.n_leaves, self.leaf_width)    # (batch_size, self.n_leaves, self.leaf_width)
        element_logits += self.b1s.view(1, *self.b2s.shape)                                 # (batch_size, self.n_leaves, self.leaf_width)
        element_activations = self.activation(element_logits)                               # (batch_size, self.n_leaves, self.leaf_width)
        element_activations = self.leaf_dropout(element_activations)                        # (batch_size, self.n_leaves, self.leaf_width)

        new_logits = torch.empty((batch_size, self.n_leaves, self.output_width), dtype=torch.float, device=x.device)
        for i in range(self.n_leaves):
            element_logits[:, i] = torch.matmul(
                element_activations[:, i],
                self.w2s[i]
            ) + self.b2s[i]
    ...
    def eval_forward(self, x: torch.Tensor, leaf_ids:bool=False) -> torch.Tensor:
        ...
        for i in range(leaves.shape[0]):
            ...
            x_h += self.b1s[leaf_index].unsqueeze(-2)    # (1, self.leaf_width)
            x_h = self.activation(x_h)           # (1, self.leaf_width)
            logits = torch.matmul(
                x_h.unsqueeze(0),                  # (1, self.input_width)
                self.w2s[leaf_index]                # (self.input_width, self.leaf_width)
            )                                               # (1, self.leaf_width)
            logits += self.b2s[leaf_index].unsqueeze(-2)    # (1, self.leaf_width)
            activations = self.activation(logits)           # (1, self.leaf_width)
            new_logits[i] = torch.matmul(
                activations,
                self.w3s[leaf_index]
