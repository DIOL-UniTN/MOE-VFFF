...
class RandomVirt():
    ...
    def evaluate(self):
        ...
        x = np.empty((self.pop_size, self.n_var), dtype=int)
        x[:, 0] = np.random.randint(10, 10000, size=self.pop_size)
        x[:, 1] = np.array([np.random.randint(1, self.max_params/page_size) for page_size in x[:, 0]])
